...
Guido va Rossum: Utilize a função reduce() se você realmente precisar dela. Em todo caso,
...
# E vocẽ tem uma função que recebe dois parâmetros:
...
Ou seja, em cada passo ela aplica a função passando como primeiro argumento o resultado da aplicação anterior. No final, reduce()
irá retornar o resultado final.

Alternativamente, poderiamos ver a função reduce() como:

funcao(funcao(funcap(a1, a2), a3), a4), ...), an)
"""
...
dados = [2,3,4,5,7,11,13,17,19,24,28,54]
...
# É mais preferivel usar do que o reduce
res = 1
for n in dados:
    res = res * n
